...
mod commands {
    ...
    pub struct ZipProgressPayload {
        /// 0-based index of the file just written.
        pub index: usize,
        pub total: usize,
        pub zip_path: String,
    ...
    /// Build a ZIP archive from `entries`, writing directly to `output_path`.
    ///
    ...
    ///   No full-file or full-archive buffers are allocated.
    /// • Emits `zip://progress` (after each successful file) and `zip://skip`
    ///   (when a file cannot be opened) events so the UI can update in real
    ///   time.
    ///
    /// Returns the number of entries successfully written.
    /// Async wrapper: the actual blocking IO runs in Tauri's `spawn_blocking`
    /// thread pool so the async runtime (and the JS event loop) stay free to
    /// deliver the `zip://progress` / `zip://skip` events that are emitted
    /// from inside the worker thread.  Without this the JS `invoke` future
    /// would hold the runtime hostage for the entire duration of the ZIP build
    /// and none of the intermediate events would ever be received.
    #[tauri::command]
    pub async fn build_zip(
        ...
        spawn_blocking(move || build_zip_blocking(app, output_path, entries))
            .await
