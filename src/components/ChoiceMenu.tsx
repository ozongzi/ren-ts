...
import { stripRenpyTags } from "../textParserUtils";
...
export const ChoiceMenu: React.FC<ChoiceMenuProps> = ({
  ...
  // Defer the update to a microtask so we avoid calling setState synchronously
  // inside an effect body (satisfies react-hooks/set-state-in-effect).
  useEffect(() => {
    if (choices.length === 0) return;
    Promise.resolve().then(() => setFocused(0));
  }, [choices]);
