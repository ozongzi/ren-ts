...
- **iOS**：可以直接使用 [Web 版](https://ren-ts.fhmmt.games/)，用 Safari 打开网址即可，无需安装。但受限于 iOS 的浏览器限制，Web 版**无法使用脚本转换工具**，需要提前在电脑上转换好脚本再导入。存档功能通过浏览器内置的 OPFS 实现，Safari 15.2+ 均可正常使用。想体验完整功能需自行编译安装（见下方说明）
...
转换工具内置两套独立的翻译方案，在「转换」面板里勾选对应选项即可启用。两套方案互斥，同时只能启用一个。

---

### 方案一：导入 Ren'Py 翻译文件

适合游戏官方已提供中文翻译、翻译文件放在 `tl/` 目录下的情况。

**使用方法：**

1. 勾选「导入翻译」
2. 在「翻译目录」填入翻译文件所在路径，例如 `tl/chinese`（相对于 `game/` 目录）
3. 点击转换，转换器会自动读取该目录下所有 `.rpy` 翻译文件，在转换过程中直接将对话文本替换为译文
...
> `tl/` 目录本身不会被转换或打包进输出 ZIP，只作为翻译来源读取。

---

### 方案二：LLM 机器翻译

适合游戏没有官方翻译、想用 AI 自动翻译的情况。翻译结果会持久化缓存在本地，下次打包时无需重新调用 API。

**使用方法：**

1. 勾选「LLM 翻译」
2. 填入 API Key（兼容 OpenAI 接口格式）
3. 按需调整端点地址、模型名称、目标语言、批量大小、并发数、系统提示词
4. 点击转换——脚本转换完成后自动进入翻译阶段，按批次调用 LLM，完成后将译文注入所有脚本文件再打包

**缓存机制：**

- 每条译文按原文 key 缓存在浏览器 OPFS 中，以游戏目录名区分
- 翻译进度实时持久化，中途中断后再次打包会自动跳过已缓存条目，只翻译新增文本
- **没有 API Key 时也可打包**：只要缓存已有内容，打包时会直接将缓存译文注入，无需再次调用 API

**缓存管理按钮：**

| 按钮 | 说明 |
|------|------|
| 导出 JSON | 将当前缓存导出为 `{ "原文": "译文" }` 格式的 JSON 文件，可备份或在不同设备间共享 |
| 导入 JSON | 从本地 JSON 文件合并翻译条目到缓存（新文件内容覆盖已有同 key 条目，缓存中有但 JSON 里没有的条目保留不动） |
| 导出未翻译 | 导出所有尚未翻译的原文条目（值为空字符串），填写后可再次导入 |
| 清除缓存 | 清空当前游戏的全部翻译缓存，不可撤销 |
...
**Q：iOS 怎么安装？**  
A：推荐先试试 [Web 版](https://ren-ts.fhmmt.games/)，用 Safari 打开网址即可，不需要安装任何东西。存档功能通过 OPFS 实现，Safari 15.2+ 均正常支持。主要限制是**无法在 iOS 上直接转换游戏脚本**（File System Access API 在 iOS Safari 不可用），需要先在电脑上用桌面版或 Chrome/Edge 转换好 `.zip`，再在 iOS 上打开。如果想要完整的转换体验，需要自己用 Xcode 编译安装，对普通用户门槛较高。由于上架 App Store 需要每年 ¥688 的苹果开发者账号，暂时无法直接分发安装包，如果你愿意赞助欢迎联系 👀
